...
8. **simple_web in SCOOP mode** (new, dependency task 0) — the 2026-08-29 spike: with `use="scoop"` simple_web fails VFFD(8) on four `once ("PROCESS")` singletons (`SIMPLE_WEB_SERVER_ROUTER.routes`, `.middleware_pipeline`, `SIMPLE_WEB_SERVER.router`, `SIMPLE_WEB_SERVER_EXECUTION.router`). Everything else in the stack type-checks under SCOOP, and EWF's httpd ships a real SCOOP connector (`httpd/concurrency/scoop`: a `CONCURRENT_POOL` of `separate HTTPD_REQUEST_HANDLER`, sized by `max_concurrent_connections`). Nothing can be implemented until simple_web builds and runs there.
